...
def list_mappings(db: Session = Depends(db_session)) -> list[AttributeMapping]:
    return db.query(AttributeMapping).order_by(AttributeMapping.attribute_code, AttributeMapping.human_value).all()


@router.post("", response_model=AttributeMappingRead)
def create_mapping(payload: AttributeMappingCreate, db: Session = Depends(db_session)) -> AttributeMapping:
